...
from vimba import Vimba, VimbaFeatureError, Camera, Frame, intersect_pixel_formats, OPENCV_PIXEL_FORMATS, PixelFormat
...
class AlliedVisionCamera(QObject):
    ...
    def frame_handler(self, cam: Camera, frame: Frame):
        try:
            ...
            height, width, depth = self.latest_frame_data.shape
            # put logging text:
            self.logging.remove()
            self.logging.set_info(f'Resolution: {width}x{height}')
